...
> **"逐字节一致"曾有一处明确的例外**：§2.4.4 那条"错误被自己的收尾洗成成功"是**缺陷
> 而非契约**（`error` 之后直接 `message_stop`，不补
> `message_delta(end_turn)`）。凡属"错误被伪装成成功"的行为一律不复刻 —— 这正是
> 本 spec 反复强调的最坏失败模式。**该缺陷已在 `27df44c` 修掉，golden 已更新**，
> 对 Rust 侧不再构成例外 —— 照 golden 复刻即为正确行为。
...
#### 2.4.4 更糟的一种形状：错误之后补终止记录会伪装成"正常说完"（已修复）

代理**曾经**的流级错误处理是 `error` 之后**再补 `finishRecords("end_turn")`**。
实测这条完整序列的客户端结果是：
...
> **已修复**（`27df44c`，2026-09-15）：`AnthropicStreamEncoder.errorRecords()` 现在
> 发 `error` + `message_stop`、**不再补** `message_delta(end_turn)`，且在 `error`
> 之前先关闭已开的内容块。`conformance/golden/behaviour.json` 的
> `stream/*/error-after-content` 已锁定新行为（`error` 后直接 `message_stop`）。
> **Rust 侧照 golden 复刻即可；不要"顺手加固"把 `message_delta` 补回来。**
...
| 参数 | 默认 | 上限 | 语义 |
| ---- | ---- | ---- | ---- |
| `CC_UPSTREAM_TIMEOUT_MS` | 600000 (10 min) | 30 min | 建连 + 请求写入的期限（**不是**读期限） |
| `CC_IDLE_TIMEOUT_MS` | 120000 (2 min) | 30 min | 流中**相邻字节块**的最大间隔；`0` 禁用 |
| `CC_NO_OUTPUT_TIMEOUT_MS` | 30000 (30 s) | 30 min | **首字节前**完全无字节的窗口；超过则丢弃本次尝试并**从头重发**；`0` 禁用 |
| `CC_NO_OUTPUT_RETRIES` | 3 | 10 | 静默尝试最多重发几次；`0` 禁用 |

- `CC_NO_OUTPUT_*` 是独立的快速重试通道：只作用于"零字节"阶段，与 5xx/429 的快速
  退避重试**并存且互不干扰**。首字节一旦到达，该通道立即失效，改由 idle 超时 +
  已有的 splice 机制接管（此时重发会重复客户端已看到的内容）。
...
| **错误被自己的收尾洗成成功** | 流级错误后补 `finishRecords` → 客户端 `finishReason` 变 `stop`，"断流"看起来像"说完了"（§2.4.4，实测） | **已在 `27df44c` 修复**：`errorRecords` 只发 `error` + `message_stop`；golden 已锁定新行为，Rust 侧照抄，勿回补 `message_delta` |
